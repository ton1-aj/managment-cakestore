...
<div class="container mt-4">
    ...
    <div class="card mb-4 shadow-sm">
        <div class="card-header bg-primary text-white">Tambah Kue</div>
        <div class="card-body">
            <form action="{{ route('kue.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row mb-2">
                    <div class="col-md-4">
                        ...
                        <select name="id_kategori_kue" class="form-control" required>
                            <option value="">-- Pilih Kategori --</option>
                            @forelse($kategori as $k)
                                <option value="{{ $k->id_kategori_kue }}">{{ $k->nama_kategori_kue }}</option>
                            @empty
                    ...
                    <div class="col-md-4">
                        <label>Harga</label>
                        <input type="number" name="harga_kue" class="form-control" required>
                    </div>
                </div>

                <div class="mb-2">
                    <label>Komposisi</label>
                    <textarea name="komposisi_kue" class="form-control" required></textarea>
                </div>

                <div class="row mb-2">
                    <div class="col-md-6">
                        <label>Stok</label>
                        <input type="number" name="stok_kue" class="form-control" required>
                    </div>
                    <div class="col-md-6">
                        <label>Foto</label>
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-secondary text-white">Daftar Kue</div>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <button
                                ...
                                data-kategori="{{ $item->id_kategori_kue }}"
                            >Edit</button>

                            {{-- Tombol Hapus --}}
                            <form action="{{ route('kue.destroy', $item->id_kue) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-danger btn-sm" onclick="return confirm('Yakin ingin menghapus kue ini?')">Hapus</button>
                            </form>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="7" class="text-center">Belum ada data kue</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="modal-content">
            <form method="POST" enctype="multipart/form-data" id="editForm">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-2">
                        <label>Kategori</label>
                        <select name="id_kategori_kue" id="edit_kategori" class="form-control">
